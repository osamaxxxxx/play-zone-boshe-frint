...
export default function EditDeviceModal() {
  const { editingDevice, settings, setShowEditDeviceModal, updateDevice } = usePlayZone();
  const [number, setNumber] = useState(editingDevice?.number || '');
  const [type, setType] = useState(editingDevice?.type || 'PS4');
  const [selectedGames, setSelectedGames] = useState<string[]>(editingDevice?.games || []);
  ...
  return (
    <div className="fixed inset-0 bg-black/70 modal-overlay z-50 flex items-center justify-center p-0 md:p-4" onClick={() => setShowEditDeviceModal(false)}>
      <div className="modal-content bg-gray-900 rounded-2xl max-w-md w-full border border-purple-500/30 overflow-y-auto max-h-screen md:max-h-[90vh]" onClick={e => e.stopPropagation()}>
        <div className="p-6 border-b border-gray-700 sticky top-0 bg-gray-900">
          <div className="flex items-center justify-between">
            <h2 className="text-xl md:text-2xl font-bold">تعديل الجهاز</h2>
            <button onClick={() => setShowEditDeviceModal(false)} className="text-gray-400 hover:text-white p-2"><i className="fa-solid fa-times text-xl"></i></button>
          </div>
        </div>
        <div className="p-6 space-y-4">
          ...
          <button onClick={() => updateDevice(editingDevice.id, number, type, selectedGames)}
